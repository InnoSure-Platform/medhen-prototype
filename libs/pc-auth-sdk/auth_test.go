...
func TestValidate_RejectsAlgConfusion(t *testing.T) {
	...
	// An attacker mints an HS256 token; the RS256-only parser must reject it
...
func TestValidate_RejectsWrongKey(t *testing.T) {
	...
	// Token signed by a different key than the validator trusts.
...
func TestHandler_RejectsXTenantIDHeader(t *testing.T) {
	...
	// The old X-Tenant-ID auth bypass must no longer be honored.
...
func TestHandler_AcceptsValidToken(t *testing.T) {
	...
	if gotTenant != "eic" {
		t.Fatalf("tenant not propagated to context, got %q", gotTenant)
	}
}
