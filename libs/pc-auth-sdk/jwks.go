...
func (ks *keySet) keyByID(ctx context.Context, kid string) (*rsa.PublicKey, error) {
	...
	// Unknown kid: refresh once (rate-limited) and retry.
...
func (ks *keySet) refresh(ctx context.Context) error {
	...
	// Rate-limit refreshes triggered by unknown/forged kids.
	...
	for _, k := range jwks.Keys {
		if k.Kty != "RSA" {
			continue // this validator only supports RSA signing keys
		}
		if k.Use != "" && k.Use != "sig" {
			continue
		}
