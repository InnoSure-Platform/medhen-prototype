...
type Product struct {
	...
	UWRuleSets       []*UnderwritingRuleSet
...
func NewProduct(tenantID, code, lob, name string, requireFairValue bool) *Product {
	...
	return &Product{
		...
		RateTables:       make([]*RateTable, 0),
		UWRuleSets:       make([]*UnderwritingRuleSet, 0),
		CreatedAt:        now,
		UpdatedAt:        now,
	}
}
...
// AddRateTable adds a rate table to the product.
func (p *Product) AddRateTable(rt *RateTable) error {
	if p.Status != StatusDraft {
		return ErrProductNotInDraft
	}
	p.RateTables = append(p.RateTables, rt)
	p.incrementVersion()
	return nil
}

// AddUWRuleSet adds an underwriting rule set to the product.
func (p *Product) AddUWRuleSet(rs *UnderwritingRuleSet) error {
	if p.Status != StatusDraft {
		return ErrProductNotInDraft
	}
	p.UWRuleSets = append(p.UWRuleSets, rs)
	p.incrementVersion()
	return nil
}
